    ...
    <div class="lex flex-col w-full max-w-[420px] gap-8">
      <div class="max-w-[320px] md:max-w-[400px] w-full">
        ...
        <CardContent>
          ...
          <form
            v-else-if="step === 'answer_question'"
          ...
          <div v-else-if="step === 'otp_consent'" class="flex flex-col gap-3">
            <p class="text-sm text-muted-foreground text-center">
              We'll send a verification code to the email on file for
              <span class="font-medium text-foreground">{{ identifier }}</span
              >.
            </p>
            <p v-if="formError" class="text-sm text-destructive text-center">{{ formError }}</p>
          </div>
        ...
        <CardFooter class="flex flex-col gap-3">
          ...
          <Button
            v-else-if="step === 'otp_input' && otpSecondsLeft > 0"
            class="w-full"
            @click="submitOtp"
            :disabled="isLoading"
          >
